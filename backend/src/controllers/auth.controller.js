...
// generic session endpoint: works for both normal users and food partners
async function getSession(req, res) {
    ...
    try {
        ...
        if (user) {
            return res.status(200).json({ type: 'user', user: { _id: user._id, email: user.email, fullname: user.fullname } });
        }
        // try food partner
        const partner = await foodPartnerModel.findById(decoded.id);
        if (partner) {
            return res.status(200).json({ type: 'foodPartner', foodPartner: { _id: partner._id, email: partner.email, name: partner.name } });
        }
